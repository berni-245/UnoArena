...
- **INV-G-19 (First Card Special Handling):** If the first card flipped from the Draw Pile at Game start is a Wild Draw Four, it is returned to the Draw Pile, the pile is reshuffled, and a new first card is drawn. This process repeats until a non-Wild-Draw-Four card is revealed. This is the only automatic reshuffle that does not require a new seed recording (it occurs during `Initializing` phase before any player-visible game state exists).
