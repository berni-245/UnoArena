...
| 2 | **Match** | A best-of-three series of Games played by the same set of players within a single Room. In 2-player rooms, the first player to win 2 Games wins the Match. In multi-player rooms (3–10 players), all 3 Games are played (unless the top-3 advancement outcome is mathematically determined earlier — see INV-M-04), and players are ranked by: (1) Games won, (2) cumulative Card Points (lower is better), (3) earliest Completion Time. In tournament rooms, the top 3 in this ranking advance. | RG | A Match exists only as a grouping of Games within a Room. It is not the same as a Round (tournament elimination tier). In casual rooms, a Match is still best-of-three but carries no tournament consequence. |
| 3 | **Room** | A bounded play area hosting 2--10 players who compete in a single Match (best-of-three Games). Every Room has a lifecycle with five states (Waiting, Ready, In-Progress, Completed, Abandoned) and a unique identifier. | RG | See Casual Room vs. Tournament Room for behavioral differences. A Room is not a Round. |
...
| 35 | **Waiting** (Room State) | The initial Room State. The Room is open for players to join. Transitions to Ready when the minimum player count is met. | RG | A Waiting room has no Games in progress. Players may join in this state. Transitions back from Ready to Waiting if players leave and drop the count below minimum. |
| 36 | **Ready** (Room State) | The Room State after the minimum player count is met but before the host has started the Match. The host may issue `StartMatch` from this state. Players may still join until `maxPlayers` is reached. | RG | Distinct from Waiting: the room has enough players to begin but remains open to additional players. A player leaving may drop it back to Waiting. |
...
| 38 | **Completed** (Room State) | The terminal Room State reached when the Match concludes with a valid outcome. Results are finalized and emitted. No further state changes are permitted. | RG | A Completed room is immutable. Events from this room are consumed downstream by Ranking and/or Tournament Orchestration. |
...
| 41b | **Host** | The player who created a casual Room. The Host is the only player who may issue the `StartMatch` command. Host status is assigned at Room creation and is not transferable. Tournament rooms have no Host; they are auto-started by the system. | RG | Host is a casual-room concept only. See A18 for the distinction between player-created and system-created rooms. |
