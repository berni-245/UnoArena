...
| 72 | **Game Log** | The append-only, immutable record of every state change within a Game: card plays, draws, Uno calls, challenges, penalties, forfeits, and system actions (auto-skip, reshuffle). Every entry includes a timestamp and causal event reference. | RG | The Game Log is the source of truth for replay and dispute resolution. It is never modified after writing. |
| 73 | **Immutable Event Log** | The platform-wide, append-only log of all domain events across all bounded contexts. Each event is recorded before being broadcast. The Game Log is a subset scoped to a single Game. | RG, TO, RK, IS | The Immutable Event Log is a system-level concept broader than the Game Log. |
| 74 | **Idempotency Key** | A unique identifier attached to a command by the client to allow safe retries. If the server has already processed a command with the same Idempotency Key, it returns the cached result without re-executing. | RG | Idempotency Keys prevent duplicate processing; they are not the same as Sequence Numbers (which enforce ordering). |
| 75 | **Server-Authoritative RNG** | The principle that all randomness (deck shuffling, draw order) is generated exclusively on the server. Clients never influence or provide random seeds. | RG | This is a design invariant, not a runtime entity. |
| 76 | **Event Ordering** | The guarantee that events within a single Game are totally ordered and causally consistent. Cross-game events are eventually consistent. | RG, TO | Within a Game, ordering is strict. Across Rooms or Contexts, ordering is best-effort and event-driven. |
...
| # | Term | Definition | Context | Disambiguation |
|---|------|------------|---------|----------------|
| 77 | **Spectator** | A connected user observing a Game in real time without participating. Spectators cannot issue game commands and are restricted to the Public Game State view. | SV | A Spectator is not a Player in the Room. A Player in the Room who has been eliminated/forfeited does not become a Spectator; they simply disconnect or wait. |
| 78 | **Observer** | Synonym for Spectator within UnoArena. Both terms may be used interchangeably in documentation, but "Spectator" is the canonical code-level term. | SV | -- |
| 79 | **Public Game State** | The subset of Game state that is safe to expose to Spectators: player names, player card counts (number of cards, not identities), Turn direction, current Top Card, Discard Pile history, and game phase. | SV | Public Game State explicitly excludes all Hand contents. |
| 80 | **Private Hand** | A player's Hand contents, which are never included in the Spectator View or any public broadcast. Only the owning player and the server have access. | RG, SV | The boundary between Private Hand and Public Game State is a hard security invariant, not a UI preference. |
| 81 | **Spectator View** | The projected read model consumed by Spectators. It is derived from domain events but filtered to exclude Private Hand data. Updates are pushed in near real time. | SV | The Spectator View is a downstream projection, not the authoritative game state. |
| 82 | **Card Count** | The number of cards in a player's Hand, visible to all players and Spectators. It reveals Hand size but not Hand contents. | RG, SV | Card Count is public; card identities are private. |
...
| # | Term | Definition | Context | Disambiguation |
|---|------|------------|---------|----------------|
| 83 | **Rate Limit** | A server-enforced cap on the number of commands or requests a client may submit within a time window. Applied at multiple layers: per IP, per User, and per Room/Tournament action type. | IS, RG | Rate limiting is a cross-cutting concern; the Identity & Session context owns the policy, but enforcement occurs at the Room Gameplay boundary. |
| 84 | **Adaptive Throttling** | A dynamic rate-limiting strategy that adjusts thresholds based on current system load, player behavior patterns, and abuse signals. Repeat offenders receive progressively stricter limits. | IS | Adaptive Throttling is a refinement of Rate Limiting, not a separate mechanism. |
| 85 | **Session Invalidation** | The act of terminating a Player's Active Session, either because a new Session was created (single-active-session enforcement) or due to administrative action (ban, security incident). | IS | Session Invalidation is distinct from Disconnection. Invalidation revokes auth; Disconnection is a connectivity loss. An invalidated session cannot reconnect. |
| 86 | **Audit Log** | A tamper-resistant record of sensitive operations: login/logout, session invalidations, administrative actions, Elo adjustments, and tournament result overrides. Separate from the Game Log. | IS, AL | The Audit Log is for security and compliance. The Game Log is for gameplay replay. They may share storage infrastructure but have different access controls and retention policies. |
| 87 | **Event Signature** | A cryptographic signature attached to critical domain events (e.g., game results, Elo updates, tournament advancement) to ensure integrity and non-repudiation. | RG, RK, TO | Event Signatures are a security measure, not a domain-logic concept. They protect against tampering of the Immutable Event Log. |
| 88 | **Input Validation** | Server-side verification that every command conforms to expected schema, value ranges, and business preconditions before processing. Invalid inputs are rejected before reaching domain logic. | RG, IS | Input Validation is the first line of defense; domain invariants are the second. Both must pass for a command to succeed. |
| 89 | **Single-Active-Session Invariant** | The system-wide rule that each Player may have at most one valid Session at any time. Enforced at login: creating a new Session immediately and atomically invalidates any prior Session. | IS | This invariant prevents multi-device abuse and session-duplication attacks. |
...
| # | Term | Definition | Context | Disambiguation |
|---|------|------------|---------|----------------|
| 90 | **Domain Event** | A record of something meaningful that happened in the domain: a card was played, a game completed, a player was eliminated. Domain Events are immutable, timestamped, and the primary integration mechanism between bounded contexts. | All | A Domain Event is not a command (which requests a change) or a query (which reads state). |
| 91 | **Command** | A request to change domain state, issued by a player or by the system (e.g., auto-forfeit). Commands are validated, sequenced, and either accepted (producing Domain Events) or rejected. | RG, TO | Commands are imperative; events are past-tense facts. |
| 92 | **Aggregate** | A cluster of domain objects treated as a single unit for consistency purposes. Each Aggregate enforces its own invariants and processes commands atomically. In UnoArena, the Room (containing Match and Games) is a primary aggregate. | All | An Aggregate is a DDD modeling concept, not a runtime service. |
| 93 | **Bounded Context** | A linguistic and model boundary within which terms have precise, consistent meanings. UnoArena defines at least six bounded contexts (see abbreviation table above). | All | The same word (e.g., "completed") may have different implications in different bounded contexts. |
| 94 | **Eventual Consistency** | The consistency model applied between bounded contexts. When Room Gameplay emits a GameCompleted event, the Ranking context will eventually process it, but not instantaneously. Within a single aggregate, consistency is immediate (strong). | All | Eventual consistency is inter-context; strong consistency is intra-aggregate. |
| 95 | **Saga / Process Manager** | A coordination pattern that manages long-running, multi-step business processes spanning multiple aggregates or contexts. Tournament Round Progression is a primary saga in UnoArena. | TO | A Saga is not a transaction; it uses compensating actions rather than rollbacks. |
| 96 | **Compensation** | A corrective domain action taken when a step in a saga fails. For example, if advancement results cannot be persisted for a Round, a compensation event may freeze the tournament and alert administrators. | TO | Compensation is not "undo"; it is a domain-meaningful corrective action. |
| 97 | **Projection / Read Model** | A denormalized, query-optimized view built by consuming Domain Events. Bracket visualizations, player statistics, and the Spectator View are all projections. | AL, SV | Projections are eventually consistent and can be rebuilt from the event log at any time. |
