...
| **Failure mode** | Redis unavailable → directive skipped (fail-open; availability preferred over throttling). Layer 1 per-IP and Layer 2 per-user limits remain active. **Redis unavailability fallback:** When Redis is unavailable and adaptive directives cannot be enforced, the gateway falls back to checking the `game-engine`'s in-memory invalidated-session set — which is already populated from Kafka `identity.sessions` consumption of `PlayerBanned` and `PlayerSuspended` events. Because banned/suspended players have their sessions invalidated (emitting `SessionInvalidated`), those sessions are present in the `game-engine`'s invalidated-session set and will fail the per-command session validation check (see `identity-session.md` §Per-Session Command Validation). This provides a secondary enforcement path: even without Redis-backed adaptive throttle directives, banned/suspended players whose sessions are invalidated will be rejected at the session-validation layer. The gap is limited to the window between ban and session invalidation propagation (~200ms Kafka lag). |
