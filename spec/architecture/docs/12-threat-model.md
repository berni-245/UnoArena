...
| S-3 | **Impersonation via forged JWT** | Attacker crafts a JWT with a fabricated `playerId` | JWTs signed with server-held secret (HMAC-SHA256 or RS256). Gateway verifies signature before accepting any claim. Unsigned or tampered tokens are rejected at the gateway before reaching any backend. | `api-gateway` |
...
| E-6 | **Privilege escalation via role claim forgery** — Attacker forges a JWT with `role: admin` | Forged JWT | JWTs are signed with server-held secret (S-3). `api-gateway` verifies signature before trusting any claim. Admin/operator/compliance roles are set at issuance time by `identity-service` and cannot be modified client-side. | `api-gateway`, `identity-service` |
...
| D-1 (command flood) | Medium | Medium | **Low** | Per-player rate limit at gateway |
