...
- **INV-R-08:** The Room transitions to `Completed` only after the Match entity signals match completion. In 2-player rooms this occurs when one player wins 2 Games; in multi-player rooms (3–10 players) this occurs after all 3 Games are played and a final ranking is established (see INV-M-04 and A15/A16), or when the last-player-standing condition is met (A17).
...
- **INV-R-12:** All **Room-level commands** (`JoinRoom`, `LeaveRoom`, `StartMatch`, `ForfeitGame`, and other lifecycle commands targeting the Room aggregate) must carry a valid `SequenceNumber` that matches the Room's expected next value for the issuing player. Commands with stale sequence numbers are rejected with HTTP 409 Conflict (see SequenceNumber Value Object, Section 3.1.5). This invariant applies exclusively to Room-aggregate commands; gameplay commands (`PlayCard`, `DrawCard`, `PassTurn`, etc.) are governed by the Game aggregate's own sequence counter (INV-G-05), which is an independent sequence.
...
- **INV-G-05 (Sequence Number Monotonicity):** Every command that mutates Game state must carry the correct next `SequenceNumber` for that player within the context of this Game. Commands with a sequence number lower than expected are rejected with HTTP 409 Conflict (V13). Commands with a higher sequence number (gap detected) are also rejected pending reconciliation. **Exception:** `CallUno`, `ChallengeUnoCall`, and `ChallengeWildDrawFour` do not require a sequence number — they are time-sensitive commands that may be issued by any opponent within a server-enforced challenge window, and their validity is governed by the window state (INV-G-06, INV-G-24) rather than turn order.
...
- **INV-G-10 (Draw Stacking):** Draw Two and Wild Draw Four effects are not stackable. The affected next player must draw the prescribed number of cards and forfeit their turn; they may not play another Draw Two or Wild Draw Four to redirect the effect.
...
- **`roomId` is assigned by Tournament Orchestration at the time the assignment is created, before the `TournamentRoomAssigned` event is emitted.** Room Gameplay does not generate the `roomId`; it uses the value from the event. This ensures that at-least-once redelivery of the event produces no duplicate rooms — the `roomId` is stable across retries.
...
- **INV-PR-05 (Elo K-Factor):** The K-factor is variable per player based on games rated (A24): K=40 for fewer than 30 completed games, K=20 for 30–99 games, and K=10 for 100 or more games. This allows new players' ratings to converge quickly while keeping experienced players' ratings stable. Delta computation uses the standard multi-player Elo extension: each player's delta is the average of N-1 pairwise expected-vs-actual comparisons against every other player in the room (A25).
...
- **INV-GL-05 (Full Private Data):** Unlike the Spectator View, the Game Log records the complete, unfiltered payload for every event, including card identities drawn, hand contents at game end, and deck seed. Access to the Game Log is restricted to authorized actors by role: support operators may read individual game logs for dispute resolution; administrators may search the full audit trail and verify integrity; compliance officers may export signed bundles for legal or regulatory requests; automated replay jobs access the log via internal service-to-service authorization. No public access is permitted.
...
- **INV-AT-05 (Retention Policy):** The audit trail is retained for a minimum of 2 years (A32). Game logs (all room types) are retained for a minimum of 1 year in active storage, after which they may be archived to cold storage but must remain queryable for compliance purposes (A32). Retention is enforced at the operational level; the domain model treats all entries as immutable within their retention period.
